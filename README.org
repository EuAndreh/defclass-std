* defclass-std
  ...
  Also, the throw away class designed in the beginning may thrive and stays the same. If only there was a way to overcome these problems... There is!
...
#+END_SRC
   ...
   =:a=, =:i=, =:r= and =:w= are connected: when all of them is omitted, =:a= and =:i= are inserted by default.
